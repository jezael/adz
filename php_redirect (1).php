<?php 
/**
 * Mobidea Redirect Tool
 * Version: 2.0
 * Description: This tool redirects your traffic to a specific website.
 * Author: Mobidea
 * Author URI:  http://www.mobidea.com
*/

function redirect_match( $f ) {

	// Immediately return true if the traffic type is set to 0 (all devices)
	if( isset($f->traffic_type) )
	{
		if( $f->traffic_type == 0 )
		{
			return true;
	...
	$redirect_matched = false;
	// check wap.wml or wap.xhtml support
	if( ( isset($_SERVER['HTTP_ACCEPT'])
			&& stristr($_SERVER['HTTP_ACCEPT'],'text/vnd.wap.wml'))
			|| ( isset($_SERVER['HTTP_ACCEPT']) && stristr($_SERVER['HTTP_ACCEPT'],'application/vnd.wap.xhtml+xml')) ) {
		$redirect_matched = true;
	}
